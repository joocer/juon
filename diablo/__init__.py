...
class Diablo(object):
    ...
    __slots__ = ['graph', 'active_nodes', 'cached_active_nodes', 'edges_cache', 'nodes_cache']

    def __init__(self, graph, active_nodes=None):
        self.graph = graph

        if active_nodes:
            # sets are faster to do 'in' against
    ...
    def values(self, key):
        ...
        """
        return list({y.get(key) for x,y in self.cached_active_nodes()})
